...
export default function Register() {
    ...
    function AddToRecoil(e) {
        ...
        for(let i=0; i<create.length ; i++){
            ...
            //    }
            //       //return setEmailAlert('Log-inned Succesfully')
    ...
    function captureEmailInput(e) {
        ...
        const emailRegExpresion =  /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/i;
        if(emailRegExpresion.test(emailInput)) {
            setEmailAlert(true)
        }else if(!emailRegExpresion.test(emailInput) && emailInput!=='') {
